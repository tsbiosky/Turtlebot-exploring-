...
def get_gain(grid,x,y,reso):
	...
	for h in range(up,down):
		for w in range(x-(int(4/reso)-h+y),x+(int(4/reso)-h+y)):
			try:
				if grid[w][h]==-1:
						gain+=1
			except:
...
def explore():
	...
	current_grid_x=(current_x-ori_x)/reso
	current_grid_y=(current_y-ori_y)/reso
	...
	for i in range(0,len(front_cell)):
		...
		if gain>=max_gain:
			...
			t_y=y
			max_gain=gain
	target_x=t_x*reso+ori_x
	target_y=t_y*reso+ori_y
	...
	if move_dis<0.3:
		...
		rad=np.arctan(rad)
		#print rad
		t_rad=np.arctan((target_x-current_x)/(target_y-current_y))
		cmd_vel=rospy.Publisher('cmd_vel_mux/input/navi',Twist,queue_size=10)
		r=rospy.Rate(1);
		move_cmd=Twist()
		move_cmd.linear.x=0
		move_cmd.angular.z=-(t_rad-rad)/2
		for i in range(0,2):
			cmd_vel.publish(move_cmd)
			r.sleep()
		move_cmd.linear.x=((target_x-current_x)**2+(target_y-current_y)**2)**0.5*0.5
		move_cmd.linear.y=0
...
def getpos(msg):
	...
	#print yaw_x,yaw_y,yaw_z,yaw_w,rad
